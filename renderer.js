const successAudio =new Audio('./assests/soun/sucess.mp3')
const errorAudio =new Audio('./assests/soun/error.mp3')
const trashAudio =new Audio('./assests/soun/trash.mp3')
...
class ClipBOARD extends React.Component{
    ...
    loadHistory = () => {
        const historyLocal = JSON.parse(window.localStorage.getItem('ClipBOARD')) || []
        return historyLocal
    }

    updateLocalStorage = () =>{
        window.localStorage.Storage.removeItem('ClipBOARD')
        window.localStorage.setItem('ClipBOARD', JSON.stringify(this.state.history))
    }

    setLastCopiedText = (text) => window.localStorage.setItem('ClipBOARD:last-copied',text)

    getLastCopiedText = ()=>window.localStorage.getItem('ClipBOARD:last-copied')
    ...
    startClipboardWatch = () => {
        ...
        this.state.interval = setInterval(() => {
          ...
          if (this.state.StorageExceed) {
            ...
            this.setState({ StorageExceed: false })
          ...
          if(text.trim().length > maxCopy) {
            ...
            M.toast({ html: `Cannot copy text of characters more than ${maxCopy}` })
          ...
          if (this.state.history.length >= storageLimit) {
            ...
            this.setState({ StorageExceed: true })
            return
          }
      ...
      handleClearStorage = () => {
        ...
        // Clear the last copied text to avoid conflicts
        window.localStorage.removeItem('ClipBOARD:last-copied')
      }
          ...
          <React.Fragment>
            <nav>
              <div class="nav-wrapper brown darken-3">
                <a class="brand-logo">ClipBOARD 📋</a>
                <ul class="right">
